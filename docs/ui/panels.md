...
- **The conversation makes room for a lip.** A lip is the one part of a sheet that shares the screen with the conversation instead of covering it, so `[data-nest]` is padded by exactly the lip (`--nest-lip-b` / `--nest-lip-r`, published by `MobilePanelSwitcher`) and eases over the 300ms the card settles in. Reserved while the lip exists rather than while it is visible: open, the card covers the conversation anyway, so tying it to `open` would animate something nobody can see.
